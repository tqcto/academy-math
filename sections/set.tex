...
\begin{document}
  ...
  \section{集合論}
    ...
    \subsection{写像}
      集合間を関係付ける概念として\textbf{写像}がある。ここではこの写像についてみていこう。
      ...
      \subsubsection{写像の定義}
        ...
        また、$\ds X$の要素$\ds x$を写像$\ds f$によって写した$\ds Y$の要素が$\ds y$であることを、\textbf{$\ds x$を写像$\ds f$によって移した像}という。\par
        ...
        \paragraph{定義域・値域}
          ...
          \footnote{
            文面上では勘違いしやすいが、一般には$\ds fの値域 = Y$ではなく$\ds fの値域 \subset Y$である。
          }\par
