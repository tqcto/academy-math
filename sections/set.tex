...
\begin{document}
  ...
  \section{集合論}
    ...
    \subsection{写像}
      ...
      \subsubsection{恒等写像}
        写像$\ds f: X \mapsto Y$について, $\ds X = Y$であるとき, すなわち
        
        \begin{center}
          \begin{equation*}
            \begin{split}
              $\ds \forall_{x \in X} [ f(x) = x ]$
            \end{split}
          \end{equation*}
        \end{center}
        
        が成立するような写像$\ds f$を\textbf{恒等写像}という。
        つまり, すべての元をそっくりそのまま写すような写像のことである。\par
        例えば, 写像$\ds f(x) = x$は恒等写像である。
      ...
      \subsubsection{単射と全射}
        ...
        \paragraph{単射}
          ...
          例えば, $\ds x \in \mathbb{R}$とするとき, 写像$\ds f(x) = e^x$は$\ds f: \mathbb{R} \mapsto \{ y \in \mathbb{R} \mid y > 0 \}$であり, 
          任意の値域の要素は定義域の要素と一対一に対応するので単射である。
          \footnote{
            ただし, 任意の$\ds y \in \mathbb{R}$は$\ds y \leq 0$を含むためこの区間の要素は原像と対応せず, 全体としては一対一に対応していないことがわかる。これが単射と全単射の違いである。\par
            では, $\ds \{ x \in \mathbb{R} \mid x > 0 \}$を定義域とするとどうだろうか。
            この場合, $\ds y$全体の集合も$\ds \{ y \in \mathbb{R} \mid y > 0 \}$であるから$\ds f$は全射性も持つため全単射であると言える。
          }
        ...
        \paragraph{全射}
          ...
          例えば, $\ds x \in \mathbb{R}$とするとき, $\ds f(x) = x^2$は$\ds \forall_{y \in Y} [ \exists_{x \in X} [ f(x) = y ] ]$が成立するので全射である。
          \footnote{
            しかし, 任意の$\ds x_1, x_2 \in \mathbb{R} \land x_1 \neq x_2$に対し必ずしも$\ds f(x_1) \neq f(x_2)$が成立するわけではないので, これは単射性を持たず全単射ではない。
            反例の一つとして, $\ds x_1 = -1 \land x_2 = 1$のとき$\ds f(x_1) = f(x_2) = 1$であることである。\par
            これは, 例えば定義域が$\ds \{ x \in \mathbb{R} \mid x \geq 0 \} (\eqqcolon X')$ならば$\ds \forall_{x_1, x_2 \in X'} [ x_1 \neq x_2 \Longrightarrow f(x_1) \neq f(x_2) ]$
            が言え, 単射性を持つため全単射である。
          }
      ...
      \subsubsection{全単射(一対一対応)}
        ...
        例えば, $\ds x \in \mathbb{R}$の下で$\ds f(x) = x^3$は, $\ds \forall_{x_1, x_2 \in \mathbb{R}}[ x_1 \neq x_2 \Longrightarrow f(x_1) \neq f(x_2) ]$を満たすので単射性を持ち, 
        またこの写像$\ds f$の値域を$\ds Y$としたとき, $\ds \forall_{y \in Y}[ \exists_{x \in \mathbb{R}}[ y = f(x) ] ]$を満たすので全射性も持つ。
        したがって, 写像$\ds f$は全単射であることがわかる。

      \subsubsection{写像の合成}
        写像$\ds f: X \mapsto Y, g: Y \mapsto Z$としよう。
        このとき, $\ds x \in X$として2つの対応を繋げると

        \begin{center}
          \begin{equation*}
            \begin{split}
              $\ds x \stackrel{f}{\mapsto} f(x) \stackrel{g}{\mapsto} g(f(x))$
            \end{split}
          \end{equation*}
        \end{center}

        となり, これは$\ds X$から$\ds Z$への写像
        ...
        \begin{center}
          \begin{tikzpicture}[auto]
            ...
            \def \circleRadius{20pt}
            \def \xCirclePoint{(0,  0)}
            \def \yCirclePoint{(50pt, 25pt)}
            \def \zCirclePoint{(100pt,  0)}
            \def \xCircleNamePoint{(0,  20pt)}
            \def \yCircleNamePoint{(50pt,  45pt)}
            \def \zCircleNamePoint{(100pt,  20pt)}
            \def \xItemNamePoint{(0, 5pt)}
            \def \yItemNamePoint{(50pt, 30pt)}
            \def \zItemNamePoint{(100pt, 5pt)}

            %Xのベン図
            \draw \xCirclePoint circle (\circleRadius);
            \node[set label,  text=black] (X) at \xCircleNamePoint {$\ds X$};

            %Yのベン図
            \draw \yCirclePoint circle (\circleRadius);
            \node[set label,  text=black] (Y) at \yCircleNamePoint {$\ds Y$};

            %Xのベン図
            \draw \zCirclePoint circle (\circleRadius);
            \node[set label,  text=black] (Z) at \zCircleNamePoint {$\ds Z$};

            \node at \xItemNamePoint {$\ds x$};
            \node at \yItemNamePoint {$\ds y$};
            \node at \zItemNamePoint {$\ds z$};

            %x->y
            \fill[fill=black] \xCirclePoint circle (1.5pt);
            \draw[->] (0,  0) to node {$\scriptstyle f$} (50pt,  25pt);

            %y->z
            \fill[fill=black] \yCirclePoint circle (1.5pt);
            \draw[->] (50pt,  25pt) to node {$\scriptstyle g$} (100pt,  0);

            %x->z
            \fill[fill=black] \zCirclePoint circle (1.5pt);
            \draw[->] (0,  0) to node{} (100pt,  0);
            \node[text=black] at (50pt, -10pt) {$\scriptstyle g \circ f$};

          \end{tikzpicture}
        \end{center}

        このような写像を$\ds f$と$\ds g$の\textbf{合成写像}といい, 

        \begin{center}
          \begin{equation*}
            \begin{split}
              $\ds g \circ f (x): X \mapsto Z$
            \end{split}
          \end{equation*}
        \end{center}
        ...
        \paragraph{合成順序}
          一般に, $\ds g \circ f$が定義されるとき, $\ds f \circ g$も定義可能であるとは限らず, 仮に定義可能であったとしても$\ds g \circ f$と$\ds f \circ g$は原則一致しない。\par
          例えば, $\ds f(x) = \sin x, g(x) = \ln x$の場合を考えてみると, 

          \begin{center}
            \begin{equation*}
              \begin{split}
                $\ds g \circ f (x) $  & $\ds = \ln ( \sin x )$\\
                $\ds f \circ g (x) $  & $\ds = \sin ( \ln x )$\\
              \end{split}
            \end{equation*}
          \end{center}

          であり, $\ds g \circ f (x) \neq f \circ g (x)$の反例であることがわかる。

        \paragraph{結合則}
          一般に, 写像の合成に関して結合則が成立する。
          つまり, $\ds f: A \mapsto B, g: B \mapsto C, h: C \mapsto D$に対し, 

          \begin{center}
            \begin{equation*}
              \begin{split}
                $\ds (h \circ g) \circ f= h \circ (g \circ f)$
              \end{split}
            \end{equation*}
          \end{center}
          ...
          \begin{proof}[\textbf{証明}]
            $\ds f, g, h$をそれぞれ集合$\ds AからB, BからC, CからD$への写像であるとし, $\ds x \in A$とする。
            このとき, 
            \begin{equation*}
              \begin{split}
                $\ds ((h \circ g) \circ f)(x)$  & $\ds = (h \circ g)(f(x))$\\
                                                & $\ds = h(g(f(x)))$\\
                                                & $\ds = h((g \circ f)(x))$\\
                                                & $\ds = h(g \circ f)(x)$\\
                                                & $\ds = (h \circ (g \circ f))(x)$\\
              \end{split}
            \end{equation*}
            である。\par
            よって示すべき命題は示された。
