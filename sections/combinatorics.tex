...
\begin{document}
  ...
  \section{組み合わせ論(場合の数)}
    組み合わせ論は離散数学の一部で、グラフ理論などがその中核を成す。
    \footnote{
      離散数学は「代数学・幾何学・解析学」の三分野のどれかに包含される分野ではない。
      それゆえ組み合わせ論は確率論に包含されるわけではない。
    }
    
    ここでは、組み合わせ論の一分野である\textbf{数え上げ組み合わせ論}と呼ばれる分野について議論しよう。

    \vspace{9pt}\\

    数え上げ組み合わせ論の主題は、考える状況における\textbf{場合の数}を計算することである。
    場合の数とは、ある状況における起こりうる事象の数のことをいう。\par
