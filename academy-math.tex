...
\part{基礎の確認}
  ...
  \section{論理学}
    ...
    \subsection{命題と条件}
      ...
      \subsubsection{命題}
        論理学が興味を持つのは、複雑な主張(statement)においても必ずその主張が"正しい"、あるいは"誤りである"と推論を打ち立てられるような主張である。
    ...
    \subsection{命題論理と述語論理}
      ...
      数である$\ds 2$と$\ds 3$を\textbf{作用する対称}とし、これらの間に加法\textbf{演算子}$\ds +$を組み込むことで$\ds 2 + 3$という演算を行い、その結果が$\ds 5$という数と等しいという\textbf{主張}を表す。\par
      これに対応するように、論証においては\textbf{作用する対称を命題}とし、\textbf{演算子を"かつ"(and)や"または"(or)あるいは"でない"(否定)}と対応させる。
  ...
  \section{写像}
    ...
    \subsection{写像の定義}
      ...
      \subsection{全射}
        ...
        \begin{equation*}
          \begin{split}
            $\ds f:X \mapsto Yが全単射 \defineProposition \forall_{x,x'\in X} [f(x) = f(x') \Longrightarrow x = x'] \wedge \forall_{y\in Y} [\exists_{x\in X} [f(x) = y]]$
          \end{split}
        \end{equation*}
